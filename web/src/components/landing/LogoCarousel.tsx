    ...
    <section className="relative py-20 overflow-hidden border-y border-border/50">
      <div className="max-w-7xl mx-auto px-6 mb-10">
        <motion.p
          ...
          className="text-center text-sm text-muted uppercase tracking-widest font-medium"
        >
      ...
      <div className="relative">
        ...
        <div className="flex animate-marquee w-max gap-16 items-center">
          {doubled.map((logo, i) => (
            <div
              key={`${logo.alt}-${i}`}
              className="flex items-center justify-center h-12 opacity-40 hover:opacity-80 transition-opacity duration-300 shrink-0"
            >
              <Image
                src={logo.src}
                alt={logo.alt}
